...
def plot_cqt_data(cqtd, sample_rate, label, ind):
    ...
    img = lbr.display.specshow(
        ...
        fmin=lbr.note_to_hz("A2"), # tries to set the lower bound of frequencies displayed on the y axis
    )
    ax.set_title(f'Constant-Q power spectrum + pitch of {label}')
    fig.colorbar(img, ax=ax, format="%+2.0f dB") # creates a color bar to map the CQT diagrams with relative strengths.
...
if not os.path.exists('./pickled_data/data.pkl') or not os.path.exists('./pickled_data/labels.pkl') or not os.path.exists('./pickled_data/dataFrame.pkl'):
    ...
    labels = [i.name for i in os.scandir('./data') if i.is_dir()] # obtain the note labels from the data folder, 42-43 of them
    labels.sort()
    ...
    for label in tqdm(labels, desc='Reading wav files'): # moves thru the 42 labels while producing a viewable progress bar
        ...
        for ind, wav in enumerate(wav_files):
            wav_data, wdf, wr, sr = read_wav(wav) # wav data, wav data in float, wave rate, sample rate respectively
            ...
            plot_cqt_data(cqt_datum, sr, label, ind + 1) # make plots of cqt data for each wav file
            _fft_data.append((freq, mag))  # Append fft freq and mag data as tuple onto list represnting fft data for "label" wav files
            _cqt_data.append(cqt_datum) # same logic as above but with cqt data for wav files under the note descfibed by "label"
    ...
    for i, label in tqdm(enumerate(labels), desc='adding data to dataFrame'): # move through all the labels
        for data in fft_data[i]: # move through all the files of each not represented by the label and its index
            df_data.append((
